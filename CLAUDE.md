...
- Unit tests: `internal/service/` (normalize, similarity, resolve, merge), `internal/api/` (all endpoints via httptest)
- Integration tests: `internal/service/` (resolve with real Postgres, merge with substitutes, concurrent race)
- Mocks: `internal/mocks/` (Querier), generated by mockery
- Service uses `db.Querier` interface for mockable DB layer
